...
void compteurPoint(int tblJeu[9][9], int tailleTbl)
{
	int i,j;
	int compteurN = 0;
	int compteurB = 0;
	...
	color( 0.0,0.0,0.0);
	string(25,400, chaineCaractereN );
	string(145,400, chaineCaractereB );
}
...
void tourIA(int tblJeu[9][9])
{
	...
	do
	{
		xFinal = customRandom(1,9);
		yFinal = customRandom(1,9);
	}while(xFinal < 1 || yFinal < 1 || xFinal > 9 || yFinal > 9 );
...
{	
	...
	placerHoshi(nbLigne);
	compteurPoint(tblJeu,9);
	
	color( 0.8,0.8,0.8);
	filled_rectangle(300, 380, 60, 40);
	color( 0,0,0);
	rectangle(300, 380, 60, 40);
	string(315, 400, "Save");
}

void redessinerWin(int nbLigne, int tblJeu[9][9],int tailleTbl)
{
...
int sauvegarder_partie(int nbLigne,int tblJeu[9][9])
{
	time_t rawtime;
	char buffer [255];
...
void mouse_clicked(int bouton, int x, int y)
{
	...
	if (x >= 300 && x <= 360 && y >= 380 && y <= 420)
	{
		printf("Vous demandez la sauvegarde\n");
		sauvegarder_partie(9,tblJeu);		
	}
	...
	if( xFinal < 1 || xFinal > 9 || yFinal < 1 || yFinal > 9 )
	{
...
int main() 
{
	...
	srand(time(NULL));
	init_win(400,440,"9X9",204,153,102);
	event_loop(9); // 9 est le nombre de lignes de notre jeu de GO
	return EXIT_SUCCESS;
